...
BEGIN
    ...
    IF test_exchange_id IS NOT NULL THEN
        RAISE LOG 'Testando atualização em exchange_id: %', test_exchange_id;
        ...
        -- Verificar se atualizou usando consulta explícita
        SELECT EXISTS (
            SELECT 1 FROM exchanges 
            WHERE id = test_exchange_id AND status = 'approved'
        ) INTO resultado_update;
        
        IF resultado_update THEN
            RAISE LOG 'Atualização bem-sucedida!';
        ELSE
            RAISE LOG 'Falha na atualização.';
        END IF;
    ELSE
        RAISE LOG 'Nenhum exchange pendente encontrado para teste.';
    END IF;
END $$;
